...
namespace API.Controllers
{
    public class UtilityClass
    {
        ...
        public async Task<bool> UserAlreadyExists(UserEntity entity)
        {
            if (await _context.Users.FirstOrDefaultAsync(user => user.UserName.ToLower() == entity.username.ToLower()) == null)
                return false;
            return true;
        }
        public async Task<bool> EmailAlreadyExists(UserEntity entity)
        {
            if (await _context.Users.FirstOrDefaultAsync(user => user.Email.ToLower() == entity.email.ToLower()) == null)
                return false;
            return true;
        }
